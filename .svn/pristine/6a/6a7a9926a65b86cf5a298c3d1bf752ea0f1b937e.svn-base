...
public class XiwooRecord2 extends AbstractCirculateRecord {
    ...
    public XiwooRecord2(RecordEntity recordBean) {
        super(recordBean);
        connectPath = "http://" + recordBean.getHost() + ":" + recordBean.getPort();
        aislePort = recordBean.getParams().get("aislePort");
        if (aislePort != null) {
            // rtmp://192.168.140.21:1935/live/ch0_1
            playPath = "rtmp://" + recordBean.getHost() + ":1935/live/ch" + aislePort + "_1";
            RunningLog.run("希沃录播播放路径:" + playPath);
        } else {
    ...
    private <T> String post(String path, Map<String, T> params, Map<String, String> header) throws IOException {
        Response response = null;
    ...
    private <T> String mapToJson(Map<String, T> params) {
        if (params != null) {
    ...
    public <T> T getRecordState() {
        ...
        String path = this.connectPath + "/api/v1/record/status";
        Map<String, String> header = new LinkedHashMap<>();
        header.put("token", this.token);
        try {
            String result = get(path, null, header);
    ...
    private boolean action(String action) {
        ...
        Map<String, Integer> bodyParam = new HashMap<>();
        switch (action) {
            case "StartRecord":
                bodyParam.put("action", 1);
                break;
            case "PauseRecord":
                bodyParam.put("action", 2);
                break;
            case "StopRecord":
                bodyParam.put("action", 0);
                break;
        }
        Map<String, String> headers = new HashMap<>();
        try {
            headers.put("token", this.token);
            String result = post(path, bodyParam, headers);
            ...
            if (statusCode == 10200) {
                ...
                result = post(path, bodyParam, headers);
                jsonObject = parser.parse(result).getAsJsonObject();
                statusCode = jsonObject.get("statusCode").getAsInt();
                if (statusCode == 10200) {
                    recordResult.setSuccess(true);
                } else {
                    recordResult.setSuccess(false);
