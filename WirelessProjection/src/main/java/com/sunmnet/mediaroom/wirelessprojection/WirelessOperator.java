...
public class WirelessOperator implements IConnectListener, IWirelessOperator<ClientDevice> {
    ...
    private void connect() {
        RunningLog.run("无线投屏开启连接！IP="+ip+"  serverPort="+serverPort);
        if (isConnecting.compareAndSet(false, true)) {
            this.mConnectionManager.connect(ip, serverPort, this);
        }
    }

    public void reRonnect() {
    ...
    public void onConnectSuccess() {
        ...
        ThreadUtils.execute(new Runnable() {
            ...
            public void run() {
                ...
                Log.i(WirelessOperator.this.getClass().getName(), "无线投屏开始授权！");
                RunningLog.run("无线投屏开始授权！");
