...
import com.sunmnet.mediaroom.tabsp.bean.LoginQrCodeRequestResult;
...
public class WaitForLoginDispatcher extends AbstractActivityUiDispatcher {
                ...
                if (tabspConfig != null) {
                    ...
                    while (!Thread.currentThread().isInterrupted()) {
                        ...
                        Response response = OkHttpUtil.get("http://" + tabspConfig.getRegisterInfo().getPlatformIp() + ":"+
                                tabspConfig.getRegisterInfo().getPlatformPort() +"/sys-svr/api/getQrLoginParam",
                                params, (Map<String, String>) null);
                        if (response.body() != null) {
                            String responseStr = response.body().string();
                            RunningLog.info("获取登录码参数请求返回:" + responseStr);
                            LoginQrCodeRequestResult loginQrCodeRequestResult =
                                    JacksonUtil.jsonStrToBean(responseStr, LoginQrCodeRequestResult.class);
                            loginQrCodeUuid = loginQrCodeRequestResult.getObj().getUuid();
                            LinkedHashMap<String, Object> qrParams = new LinkedHashMap<>();
                            qrParams.put("uuid", loginQrCodeRequestResult.getObj().getUuid()); // uuid
                            qrParams.put("type", loginQrCodeRequestResult.getObj().getType()); // type

                            String message = JacksonUtil.mapToJsonStr(qrParams);
                            RunningLog.info("登录二维码展示的Json参数:" + message);
    ...
    public void onLoginQrCodeResult(LoginQrCodeOperateResult result) {
        RunningLog.info("收到操作二维码结果:" + result.getSocketMessage().toString());
        ...
        if (result.getSocketMessage().getUuid().equals(loginQrCodeUuid)) {
            ...
            RunningLog.info("收到操作二维码结果， uuid不相符");
            ToastUtil.show(WaitForLoginDispatcher.this.activity,
                    this.activity.getString(R.string.qr_code_refreshed_scan_code_again));
        }
    }
