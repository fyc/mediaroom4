...
public class MQDataHandler {
    ...
    public String HOST = "tcp://192.168.100.181:1883";//服务器地址（协议+地址+端口号）
    public String USERNAME = "admin";//用户名
    public String PASSWORD = "public";//密码
    public static String PUBLISH_TOPIC = "device/state";//发布主题
    public static String RESPONSE_TOPIC = "device/state";//订阅主题
    /**
     * QUALITY_OF_SERVICE
    ...
    private MqttCallback mqttCallback = new MqttCallback() {
        ...
        public void messageArrived(String topic, MqttMessage message) throws Exception {
            ...
            Msg msg1 = (Msg) JsonUtils.jsonToBean(msg, Msg.class);
            switch (msg1.type) {
                case 1:
                    /*Intent intent1 = new Intent(Intent.ACTION_MAIN);
                    intent1.addCategory(Intent.CATEGORY_LAUNCHER);
                    ComponentName cn = new ComponentName("teaonly.rk.droidipcam", "teaonly.rk.droidipcam.CameraActivity");
                    intent1.setComponent(cn);
                    context.startActivity(intent1);*/
                    try {

                        openCLD("teaonly.rk.droidipcam", context);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    break;
                case 2:
                    URLPath path = (URLPath) JsonUtils.jsonToBean(msg, URLPath.class);
                    Intent intent = new Intent(context, PlayerActivity.class);
                    intent.putExtra("url", path.url);
